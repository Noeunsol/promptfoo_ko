...
- 현재 특징:
  ...
  - 25종+ 플러그인 **채점 사유(reason) 한국어화** + 한국어 거부(refusal) 탐지 + **언어 자동 탐지**(한글 비율)
  - 한국어 인젝션 템플릿(`dataKo.ts`)·한국어 init/온보딩 템플릿 내장
  - 결과를 SQLite(`~/.promptfoo/promptfoo.db`)에 저장하고 웹 리포트(`promptfoo view`/`redteam report`)로 조회
...
- **공격 성공(FAIL)** = 대상이 뚫림(취약점 발견), **공격 실패(PASS)** = 대상이 방어함.
- **Strategies가 멀티턴**(crescendo/goat 등)이면 평가 결과가 다시 Strategies로 루프백해 다음 턴 공격을 생성.
- **Memory**: 실행 결과는 로컬 SQLite(`~/.promptfoo/promptfoo.db`)에 저장 → `promptfoo view`/`redteam report`로 조회.
...
# 5) API 키: 저장소 루트 .env 에 작성 (예: OPENAI_API_KEY=sk-...)
```

- **가상환경 불필요**: Node/npm은 Python의 venv/conda 같은 가상환경이 없습니다. 의존성은 **프로젝트별 `node_modules/`에 자동 격리**되므로, "활성화"에 해당하는 건 이 디렉터리 안에 있는 것 + Node 버전 정렬(`nvm use`)뿐입니다.
...
promptfoo redteam setup              # 브라우저 위저드로 생성 (빌드된 UI)
promptfoo redteam init               # CLI 대화형으로 생성
...
- **promptfoo의 원격(cloud) 의존이 큼**: harmful·bias 및 다수 고급 플러그인·전략(goat·gcg·crescendo 등)은 promptfoo cloud(`api.promptfoo.app`)의 원격 생성에 의존합니다. 로컬 정렬 모델(`gpt-4o-mini`)은 유해 공격 프롬프트 생성을 **거부**하므로, 원격을 끄면 이 항목들은 **0건 생성**됩니다. (데이터셋 기반·한국 특화 플러그인은 로컬에서도 정상 동작)
...
- **선택적 외부 도구**: `video` 전략의 `ffmpeg`, python 프로바이더의 Python 등 — 해당 기능을 쓸 때만 필요.
